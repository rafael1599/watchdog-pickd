...
The way out is **Cmd+N**, and it works for a reason worth keeping in a test: it
is an APPLICATION command, not a 5250 keystroke. The dead screen has no say over
what Mocha does with its own menu shortcuts.
"""
...
class FakeEmulator:
    ...
    def __init__(self, screens=None, *, cmd_n_works=True, closes=True):
        self.events = []
...
def test_the_way_out_is_a_new_window_not_a_keystroke(monkeypatch):
    # Cmd+N is handled by Mocha, not by the session, which is exactly why it
    # escapes a screen that ignores what the session receives. Nothing is
    # closed and nothing of the operator's is taken away.
    monkeypatch.setenv("AS400_HARD_RESTART", "1")
    d = FakeEmulator()
    assert hard_restart(d, idle_fn=lambda: 1e9) is True
    assert d.events == ["cmd+n"]
...
def test_a_terminal_that_cannot_be_saved_is_not_poked_all_night(monkeypatch):
    ...
    assert d.events == ["cmd+n"]  # exactly one attempt
...
def test_closing_the_emulator_stays_off_unless_asked(monkeypatch):
    ...
    assert hard_restart(d, idle_fn=lambda: 1e9) is False
    assert d.events == ["cmd+n"]  # never quit
...
def test_it_gives_up_honestly_if_the_new_window_lands_nowhere(monkeypatch):
    ...
    with pytest.raises(as400_capture.AS400Disconnected):
        bootstrap_session(d, launch_wait=0, step_wait=0)
    assert d.events == ["launch", "cmd+n"]  # tried once, then stopped
