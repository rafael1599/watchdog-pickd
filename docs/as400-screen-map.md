...
**La salida, y es de Rafael (8 sep 2026): `Cmd+N`** — una ventana de sesión nueva. Funciona por una
razón que conviene entender: **es un comando de la aplicación, no una tecla del 5250**. La pantalla
muerta ignora todo lo que llegue a la *sesión*; no manda nada sobre los *menús de Mocha*.

**La limpieza va después, y es otra cosa:** `Ctrl+Shift+Tab` para pasar a la ventana muerta y
`Cmd+W` **+ ENTER** para cerrarla. **El `Cmd+W` sólo pregunta; el Enter es el que cierra.** Eso lo
convierte en el único sitio del driver que abre un diálogo a propósito — y un diálogo **bloquea
todos los Apple events posteriores**, así que las dos teclas viajan **dentro de un mismo osascript**:
no hay camino que mande el `Cmd+W` sin su Enter.

**Se abre antes de cerrar, y el orden es deliberado.** Con la ventana nueva ya abierta siempre hay
al menos dos, así que el `Cmd+W` **nunca puede ser el que cierre Mocha entero** — el caso que Rafael
avisó (*«cuando solo queda una ventana abierta cierra por completo el AS400»*) deja de existir en
vez de tener que manejarse.

**Y antes de cerrar, se lee.** Si el `Ctrl+Shift+Tab` aterriza en una ventana que **no** es la del
callejón —el operador tenía otras abiertas, el atajo se comportó distinto— no se cierra nada y se
vuelve adelante con `Ctrl+Tab`. Cerrarle una ventana a alguien por ordenar sería un fallo bastante
peor que dejar una muerta detrás.
