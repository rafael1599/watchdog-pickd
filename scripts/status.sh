...
pids="$(lsof -ti tcp:5000 2>/dev/null | tr '\n' ' ')"
if curl -s -o /dev/null --max-time 2 "$APP_URL"; then
  echo "✓ app    answering on $APP_URL (pid: ${pids:-?})"
else
  if [ -n "$pids" ]; then
    echo "✗ app    NOT answering, but port 5000 is held (stale pid: $pids) — run ./scripts/update.sh to recover"
  else
    echo "✗ app    not running (nothing on port 5000)"
  fi
fi
